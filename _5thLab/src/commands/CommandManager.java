...
public class CommandManager implements Command{
    ...
    public CommandManager(StudyGroupCollectionManager cManager, InputManager iManager, FileManager fManager) {
        ...
        addCommand("show", (argument) -> {
            if (collectionManager.getCollection().isEmpty()) System.out.print(ConsoleColor.RED_BACKGROUND+"collection is empty"+ConsoleColor.RESET);
            else {
        ...
        addCommand("remove_by_id", (argument) -> {
            ...
            if (collectionManager.getCollection().isEmpty()) throw new EmptyCollectionException("Collection is empty");
            if (!collectionManager.isIdInCollection(id)) throw new DataException(ConsoleColor.RED_BACKGROUND+argument+" is not id in collection"+ConsoleColor.RESET+"\n");
        ...
        addCommand("save", (argument) -> {
            ...
            else System.out.print(ConsoleColor.GREEN_BACKGROUND+"Data has been successfully saved :)"+ConsoleColor.RESET+"\n");
        });
        addCommand("execute_script", (argument) -> {
            ...
            if (callStack.contains(currentScriptFileName)) throw new DataException(ConsoleColor.RED_BACKGROUND+"Already in use"+ConsoleColor.RESET+"\n");
        ...
        addCommand("count_by_expelled_students",(argument)->{
            ...
            try{
                ...
                if(expelled<=0)throw new DataException(ConsoleColor.RED_BACKGROUND+"expelled students should be greater than 0 :("+ConsoleColor.RESET+"\n");
            }catch (NumberFormatException exception){
                throw new DataException(ConsoleColor.RED_BACKGROUND+"Invalid :("+ConsoleColor.RESET+"\n");
            }
            if(collectionManager.countByExpelledStudents(expelled)==0)throw new DataException(ConsoleColor.RED_BACKGROUND+"No expelled students found with value of #"+argument+ConsoleColor.RESET+"\n");
    ...
    public void runCommand(String s, String arg){
        try{
            if (! hasCommand(s)) throw new CommandException(ConsoleColor.RED_BACKGROUND+"No such command"+ConsoleColor.RESET);
