...
public class CommandManager implements Command {
    ...
    public CommandManager(StudyGroupCollectionManager cManager, InputManager iManager, FileManager fManager) {
        ...
        addCommand("execute_script", (argument) -> {
            if (argument == null || argument.equals("")){
                throw new RuntimeException(ConsoleColor.RED_BACKGROUND+"argument is empty"+ConsoleColor.RESET+"\n");
            }

//            if (callStack.contains(argument)) throw new DataException(ConsoleColor.RED_BACKGROUND+"Recursive file detected delete recursive file and try again"+ConsoleColor.RESET+"\n");
            if(callStack.contains(argument)){
                System.out.print(ConsoleColor.YELLOW_BACKGROUND+"Recursive file detected :|"+ConsoleColor.RESET+"\n");
  //              while(true){
                    System.out.print(ConsoleColor.CYAN_BOLD+"Enter 1.For execution and (As long as you enter (1) file will be executed)2.If you don't want to execute file "+ConsoleColor.RESET+"\n");
                    ...
                    switch (s){
                        ...
                        case("2")->{
                            throw new RuntimeException(ConsoleColor.GREEN_BOLD+"File won't be executed :| "+ConsoleColor.RESET+"\n");
                        }
                        default -> System.out.print(ConsoleColor.RED_BACKGROUND+"Invalid input try again :("+ConsoleColor.RESET+"\n");
                    }
 //                   if(s.equals("1"))break;
 //               }
            }
        ...
        addCommand("count_by_expelled_students", (argument) -> {
            if (argument.equals("") || argument == null) throw new DataException("argument can't be empty :(");
            else {
                long expelled;
                try {
    ...
    public void runCommand(String s, String arg) {
        ...
        } catch (CommandException | DataException | EmptyCollectionException | RuntimeException e) {
